...
class SendResult:
    ...
    # У сообщества найден собственный сайт (см. VkSendAdapter._has_external_site) — парсер иногда
    # по ошибке пропускает такую группу, хотя должен искать только сообщества БЕЗ сайта (см.
    # запрос пользователя 2026-08-25). Отправка сообщения этому лиду намеренно не выполняется;
    # вызывающий код (tasks.py/reply.py) не делает с этим флагом ничего особого — лид просто
    # уходит в обычный "failed" путь, как и любая другая причина не писать этому лиду.
    has_site: bool = False
